...
export function BuilderLayout({ children, agent }: BuilderLayoutProps) {
  ...
  const handleDeploy = () => {
    toast.promise(new Promise(res => setTimeout(res, 2500)), {
      loading: 'Encrypting and deploying to global edge nodes...',
      success: 'Agent deployed successfully to the Cloudflare Network',
      error: 'Protocol deployment failed',
    });
  };
  return (
      ...
      <header className="h-20 border-b border-primary/10 px-8 flex items-center justify-between bg-zinc-950/50 backdrop-blur-2xl z-40">
        ...
        <div className="flex items-center gap-4">
          <Button 
            ...
            onClick={() => toast.info('Progress captured in storage')}
          >
